...
An AI-powered job application framework built on [Claude Code](https://claude.com/claude-code). Clone it, fill in your profile, and let Claude evaluate job postings, tailor your CV, write cover letters, and prepare you for interviews.

## What this is

A structured workflow that turns Claude Code into a full-stack job application assistant. The core workflow (self-profiling, fit evaluation, and the drafter-reviewer application pipeline) is **language- and country-agnostic** — you configure it for your own roles, skills, and location. Job search runs through the `job-scraper` skill (WebSearch/WebFetch) across mainstream boards (hiring.cafe, LinkedIn, Indeed, Glassdoor, Wellfound, BuiltIn, We Work Remotely, and more).
...
> **Heads up:** This repo ships as a **template**. Every personal field is a `[placeholder]` you fill in with `/setup` (or by editing the files directly). It contains no real personal data.
...
Job search runs through the `job-scraper` skill using Claude's built-in WebSearch/WebFetch against mainstream job boards. No install step is required. Search queries live in `.claude/skills/job-scraper/search-queries.md` — edit them for your own roles, skills, and location.
...
│   │   │   ├── 01-candidate-profile.md # Your education, experience, skills (template)
│   │   │   ├── 02-behavioral-profile.md# Behavioral/self-assessment (template)
│   │   │   ├── 03-writing-style.md    # Tone, structure, do's and don'ts
│   │   │   ├── 04-job-evaluation.md   # Scoring framework for job fit
│   │   │   ├── 05-cv-templates.md     # LaTeX CV structure + tailoring rules
│   │   │   ├── 06-cover-letter-templates.md # LaTeX cover letter templates
│   │   │   └── 07-interview-prep.md   # STAR framework + interview questions (template)
│   │   ├── job-scraper/               # Job search orchestration
│   │   ├── daily-job-routine/         # (optional) daily recent-matches + Gmail sync
│   │   ├── auto-apply/                # (optional) browser form-fill — see ToS note above
│   │   └── upskill/                   # /upskill skill gap analysis and learning plan
│   └── agents/                        # helper subagents
├── cv/
│   └── main_example.tex               # moderncv LaTeX template (placeholders)
├── cover_letters/
│   ├── cover.cls                      # Custom cover letter LaTeX class
│   └── OpenFonts/                     # Lato + Raleway fonts
├── documents/                         # Career source materials for /setup and /expand (gitignored content)
...
├── job_scraper/                       # Scraper state (seen jobs, results) — gitignored content
└── SETUP.md                           # Detailed setup guide
```

Your generated, personal files — `applied-jobs.md`, `apply-queue.csv`, `shortlist.md`, tailored `cv/main_<company>.tex`, `credentials.md`, and the contents of `documents/` — are **gitignored** so they never get committed. See `.gitignore`.
...
| `04-job-evaluation.md` | Skill match areas, career goals, motivation filters, location rules |
